...
## Parity: formulae that still build from source

A formula generated by `brews:` installs whatever the `install:` block
names, and users who reach for `brew install --build-from-source` (or
`--HEAD`) get a formula that at least tries a source path. Earlier
gobottle releases traded that away: `def install` only ever ran
`bin.install "<binary>"`, which cannot work against a source tarball.

gobottle ≥0.8 closes the gap without giving up bottles. The generated
`def install` compiles the module — `depends_on "go" => :build`, the
`source.build` ldflags rendered as Ruby interpolations,
`system "go", "build", *std_go_args(...)` — so `--build-from-source` and
